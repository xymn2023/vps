...
echo -e "${GREEN}========================================${NC}"
echo
echo -e "${BLUE}脚本仓库: ${YELLOW}https://github.com/xymn2023/vps${NC}"
echo
...
if [ "$MODE_CHOICE" = "1" ]; then
    ...
    sed -i "s/CFIP = os.environ.get('CFIP', '[^']*')/CFIP = os.environ.get('CFIP', 'kejilion.pro')/" app.py
    echo -e "${GREEN}优选IP已自动设置为: kejilion.pro${NC}"
    ...
    read -p "请输入优选IP/域名 (留空使用默认 kejilion.pro): " CFIP_INPUT
    if [ -z "$CFIP_INPUT" ]; then
        CFIP_INPUT="kejilion.pro"
    fi
